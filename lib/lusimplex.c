...
static const uint8_t perm12[] =
     {7,4,5,7,6,3,11,1,9,11,0,5,2,5,7,9,8,0,7,6,9,
      10,8,3,1,0,9,10,11,10,6,4,7,0,6,3,0,2,5,2,10,0,3,
      11,9,11,11,8,9,9,9,4,9,5,8,3,6,8,5,4,3,0,8,7,
      2,9,11,2,7,0,3,10,5,2,2,3,11,3,1,2,0,7,1,2,
      4,9,8,5,7,10,5,4,4,6,11,6,5,1,3,5,1,0,8,1,5,4,
      0,7,4,5,6,1,8,4,3,10,8,8,3,2,8,4,1,6,5,
      6,3,4,4,1,10,10,4,3,5,10,2,3,10,6,3,10,1,8,3,2,
      11,11,11,4,10,5,2,9,4,6,7,3,2,9,11,8,8,2,8,10,7,
      10,5,9,5,11,11,7,4,9,9,10,3,1,7,2,0,2,7,5,8,4,
      10,5,4,8,2,6,1,0,11,10,2,1,10,6,0,0,11,11,6,
      1,9,3,1,7,9,2,11,11,1,0,10,7,1,7,10,1,4,0,0,
      8,7,1,2,9,7,4,6,2,6,8,1,9,6,6,7,5,0,0,3,9,
      8,3,6,6,11,1,0,0,7,4,5,7,6,3,11,1,9,11,0,5,2,
      5,7,9,8,0,7,6,9,10,8,3,1,0,9,10,11,10,6,4,7,0,6,
      3,0,2,5,2,10,0,3,11,9,11,11,8,9,9,9,4,9,5,8,3,6,
      8,5,4,3,0,8,7,2,9,11,2,7,0,3,10,5,2,2,3,11,
      3,1,2,0,7,1,2,4,9,8,5,7,10,5,4,4,6,11,6,5,1,
      3,5,1,0,8,1,5,4,0,7,4,5,6,1,8,4,3,10,8,8,
      3,2,8,4,1,6,5,6,3,4,4,1,10,10,4,3,5,10,2,3,
      10,6,3,10,1,8,3,2,11,11,11,4,10,5,2,9,4,6,7,3,2,
      9,11,8,8,2,8,10,7,10,5,9,5,11,11,7,4,9,9,10,3,1,
      7,2,0,2,7,5,8,4,10,5,4,8,2,6,1,0,11,10,2,1,
      10,6,0,0,11,11,6,1,9,3,1,7,9,2,11,11,1,0,10,7,
      1,7,10,1,4,0,0,8,7,1,2,9,7,4,6,2,6,8,1,9,
      6,6,7,5,0,0,3,9,8,3,6,6,11,1,0,0};

static const double F2 = 0; // 0.5*(Math.sqrt(3.0)-1.0);
static const double G2 = 0; // (3.0-Math.sqrt(3.0))/6.0;
static const double F3 = 1.0 / 3.0;
static const double G3 = 1.0 / 6.0;
static const double F4 = 0; // (Math.sqrt(5.0)-1.0)/4.0;
static const double G4 = 0; // (5.0-Math.sqrt(5.0))/20.0;
...
double lusimplex_noise4(double xin, double yin, double zin, double win) {
    ...
    // x<w impossible.  only the 24 indices which have non-zero entries make
    // any sense.  we use a thresholding to set the coordinates in turn from
    // the largest magnitude.  rank 3 denotes the largest coordinate.
    i1 = rankx >= 3 ? 1 : 0;
    j1 = ranky >= 3 ? 1 : 0;
    k1 = rankz >= 3 ? 1 : 0;
    l1 = rankw >= 3 ? 1 : 0;
    ...
    k3 = rankz >= 1 ? 1 : 0;
    l3 = rankw >= 1 ? 1 : 0;
    // the corner 5 has all offsets=1, so no need to compute that.
    double x1 = x0 - i1 + G4; // offsets for corner 2 in (x,y,z,w) coords
    double y1 = y0 - j1 + G4;
    double z1 = z0 - k1 + G4;
    double w1 = w0 - l1 + G4;
    double x2 = x0 - i2 + 2.0*G4; // offsets for corner 3 in (x,y,z,w) coords
    double y2 = y0 - j2 + 2.0*G4;
    double z2 = z0 - k2 + 2.0*G4;
    double w2 = w0 - l2 + 2.0*G4;
    double x3 = x0 - i3 + 3.0*G4; // offsets for corner 4 in (x,y,z,w) coords
    double y3 = y0 - j3 + 3.0*G4;
    double z3 = z0 - k3 + 3.0*G4;
    double w3 = w0 - l3 + 3.0*G4;
    double x4 = x0 - 1.0 + 4.0*G4; // offsets for corner 5 in (x,y,z,w) coords
